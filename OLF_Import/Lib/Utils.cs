...
namespace OLF_Import.Lib
{
    public static class Utils
    {
        ...
        public static string SaveAttachmentToWorkDir(Stream input, string path)
        {
            ...
            var success = StreamToFile(input, tmpFileName);

            if (success)
            {
                string pathExtrFromZip = string.Empty;
                using (var zipInputStream = new ZipInputStream(File.Open(tmpFileName, FileMode.Open)))
                {
                    ZipEntry zipEntryFromZippedFile = zipInputStream.GetNextEntry();
                    if (zipEntryFromZippedFile.IsFile)
                    {
                        pathExtrFromZip = Path.Combine(path, zipEntryFromZippedFile.Name);
                        FileInfo fInfo = new FileInfo(pathExtrFromZip);
                        using (FileStream file = fInfo.Create())
                        {
                            var bufferFromZip = new byte[zipInputStream.Length];
                ...
                if (!string.IsNullOrEmpty(pathExtrFromZip) && File.Exists(pathExtrFromZip))
                {
                    if (File.Exists(tmpFileName)) { File.Delete(tmpFileName); }
                    return pathExtrFromZip;
                }
                return tmpFileName;
            }
            return string.Empty;
